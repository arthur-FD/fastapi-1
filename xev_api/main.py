...
def sort_model(
    filters: List[filter_sql] = Body(...),
    columns: List[str] = Body(...),
    graph_columns: Optional[List[str]] = Body(...),
    metrics: List[str] = Body(...),
    granularity: Tuple[str, ...] = Body(...),
    date_filter: Optional[Dict] = Body(...),
    ingestion_date: constr(regex=r"[0-9]{4}-[0-9]{2}-[0-9]{2}") = Body(...),
